...
- **Done 2026-09-15 — 1b. Generic AI source scraper.** `Source` model + `src/lib/scrapers/generic.ts` (fetch modes CHEERIO / PUPPETEER / ICAL / RSS / JSONLD, robots.txt respected, 3 URLs per source), a fourth AI entry point `extractEventsFromWebPage`, and `/admin/sources` with a true dry-run Test button. **8 sources onboarded, 5 working, 51 events found on their first run (45 new submissions for review).** Nyack Library alone accounts for 40 — its LocalHop widget lives in an iframe, so PUPPETEER mode reads every frame. Big Red Books (Shopify, no events page), Homebody Books (Squarespace demo placeholders), and Helen Hayes Youth Theatre (dates only inside the Arts People ticketing app) are seeded but disabled with notes; the two bookstores belong in 1e's Instagram list. All five start in review mode, so the thin-day count doesn't move until Danny works through `/admin/submissions`. **Runtime warning:** the nightly run was 116–130 s for 17 scrapers and the generic sources add ~112 s (Nyack Library 70 s), putting the total near 230–245 s against Vercel's 300 s cap. Splitting the cron into two invocations — static scrapers and generic sources — is now the next infrastructure task, before more sources are added. *(Done 2026-09-18, see below; the cap is no longer a reason to hold off on sources.)*

- **Done 2026-09-18 — split the daily scrape.** `POST /api/scrape` takes `group=static` or `group=generic&batch=N&batchSize=M` (response carries `hasMore`), and `.github/workflows/daily-scrape.yml` now makes one call for the static scrapers (with the daily cleanup) and then pages through the generic sources 4 at a time, each step reported separately. Every scraper gets a wall-clock budget (90 s static; 60 s feed/HTML and 100 s Puppeteer for generic sources) and a timeout lands as an error `ScraperLog` row that Pulse shows. AI provider calls are capped at 45 s (`AI_REQUEST_TIMEOUT_MS`). Review-mode generic sources now run the AI categorizer on their submissions too.

  **Measured 2026-09-18 (local, against the production Supabase).** Static group: 17 scrapers, **46.8 s of scrape time** (Visit Nyack alone 32.2 s; 14 of the 17 finish under 3 s), 151 events found. Generic group: 5 sources, **58.2 s across three batches of 2** (24.0 + 11.9 + 22.3 s), slowest source 12.9 s. So each daily invocation now asks for well under half of Vercel's 300 s, and no single group is near the cap even with Chromium's slower cold start there. The end-to-end route timings are larger (static `totalDurationMs` 194.9 s) because the per-event `saveEvent` round trips run from a laptop to Supabase; on Vercel the function and the database are co-located, which is what the earlier 116–130 s figure reflects.

  **Headroom.** A new generic source costs one extra slot in a batch of 4, not extra risk to the run: at 4 per invocation, 20 sources is 5 invocations, and the workflow caps the loop at 10. The remaining single-source risk is Visit Nyack at 32 s inside a 90 s budget — worth watching on Pulse, not worth acting on yet.
